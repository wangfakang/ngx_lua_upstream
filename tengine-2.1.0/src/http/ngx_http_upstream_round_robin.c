    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    /////////////////////
   // ngx_log_stderr(0,"my test nginx debug messages");
    ...
    if (us->servers) {
        ...
        for (i = 0; i < us->servers->nelts; i++) {
            ...
            for (j = 0; j < server[i].naddrs; j++) {
               //ngx_log_debug1(NGX_LOG_HTTP, cf->log, 0,"mytest  name: %s  id: %d  weight: %d  max_fails: %d",server[i].addrs[j].name,server[i].id,server[i].weight,server[i].max_fails);
              //  ngx_log_stderr(0,"name: %s \t id: %d \t weight: %d \t max_fails: %d ",server[i].addrs[j].name,server[i].id,server[i].weight,server[i].max_fails);
